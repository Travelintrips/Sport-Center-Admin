 ...
 *
 * No SSR server needed — purely a build-time artifact step.
 * The SPA JavaScript takes over after the browser loads any of these files.
 */
...
const BASE_URL = (process.env.VITE_PUBLIC_URL ?? '').replace(/\/$/, '');
...
/** Route metadata — must match SEOHead.tsx exactly */
const ROUTES = [
  {
    path: '/',
...
/** Build the complete SEO meta block for a route */
...
/** Strip generic SEO tags Vite baked from index.html so we can inject per-route ones */
...
// Anchor: inject SEO block right after the viewport meta tag
...
if (!BASE_URL) {
  console.warn('\n⚠   VITE_PUBLIC_URL is not set.');
  console.warn('    canonical and og:url will be omitted from all pages.');
  console.warn('    Set VITE_PUBLIC_URL=https://sc.travelintrips.co.id before building for production.\n');
}
