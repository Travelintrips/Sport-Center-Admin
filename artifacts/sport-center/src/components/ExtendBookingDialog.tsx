...
export default function ExtendBookingDialog({
  open, onOpenChange, bookingId, orderNumber, facilityName, bookingDate, startTime, endTime, pricePerHour, facilityCloseTime, onSuccess,
}: Props) {
  const { t } = useLang();
  ...
  // Max extra hours limited by close time
  const currentEndHour = parseInt(endTime.split(":")[0]);
  const closeHour = facilityCloseTime ? parseInt(facilityCloseTime.split(":")[0]) : 23;
  const maxExtra = Math.min(4, closeHour - currentEndHour);
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Clock size={20} className="text-primary" /> Tambah Waktu</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground py-4 text-center">
            {t("Perpanjangan tidak tersedia — waktu telah mencapai jam tutup fasilitas.", "Extension unavailable — booking reaches facility close time.")}
          </p>
