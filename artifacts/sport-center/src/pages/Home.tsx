...
function ScheduleCalendar() {
  ...
  const title = anchor.toLocaleDateString("id-ID", {
    month: "long",
    year: "numeric",
  });
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-base font-bold text-secondary dark:text-white capitalize min-w-[160px] text-center">
            {title}
          </span>
          <button
            onClick={() => navigate(1)}
        ...
        <Select value={facilityId} onValueChange={setFacilityId}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("Semua Fasilitas", "All Facilities")}
            </SelectItem>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {Object.entries(SCHED_STATUS_LABELS).map(([key, label]) => (
          ...
          >
            <div
              className={`w-2.5 h-2.5 rounded-sm ${SCHED_STATUS_COLORS[key]}`}
            />
            {t(label.id, label.en)}
          </div>
        ))}
      ...
      <div className="rounded-2xl border border-border overflow-hidden bg-white dark:bg-slate-900 shadow-sm">
        ...
        <div className="grid grid-cols-7 border-b border-border bg-slate-50 dark:bg-slate-800/50">
          {dayNames.map((d) => (
            <div
              key={d}
              className="text-center text-xs font-bold py-3 text-muted-foreground uppercase tracking-wider"
            >
              {d}
            </div>
          ))}
          ...
          <div className="grid grid-cols-7">
            {days.map((day, i) => {
              ...
              const dayEvents = events.filter(
                (e) => e.date === dateStr || e.start?.startsWith(dateStr),
              );
                  ...
                  <div className="space-y-0.5">
                    {dayEvents.slice(0, 2).map((e: any) => (
                      ...
                      >
                        ...
                        {e.facilityName?.split(" ").slice(-1)[0]}
                      </div>
                    ))}
                    {dayEvents.length > 2 && (
      ...
      <p className="text-xs text-muted-foreground mt-3 text-center font-medium">
        {t(
          "Hanya menampilkan booking yang sudah dikonfirmasi.",
          "Only showing confirmed bookings.",
        )}
      </p>
    </div>
  );
...
function AnimatedCounter({
  ...
  if (highlight) {
    return (
      <div
        ref={ref}
        className="flex flex-col items-center justify-center p-6 bg-primary rounded-3xl shadow-lg shadow-primary/30 text-white"
      >
        <div className="text-4xl md:text-5xl font-black mb-2 flex items-end gap-0.5 tabular-nums">
          {prefix}
          {value.toLocaleString("id-ID")}
          <span className="text-2xl opacity-70">{suffix}</span>
        </div>
        <div className="text-sm font-bold uppercase tracking-widest opacity-90 text-center">
          {label}
        </div>
      </div>
    );
  ...
  return (
    <div
      ref={ref}
      className="flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-border/50"
    >
...
const TESTIMONIALS = [
  {
    quote:
      "Lantai lapangan futsalnya berstandar tinggi, empuk di lutut. Sistem bookingnya cepat tanpa antri. Fix jadi tempat rutin tim kami!",
    quoteEn:
      "The futsal court flooring is high-standard, gentle on the knees. The booking system is fast. Definitely our team's regular spot!",
    name: "Bima Arya",
  ...
  {
    quote:
      "Transit 4 jam nunggu pesawat? Nge-gym di sini solusinya. Alatnya lengkap, shower room-nya sekelas hotel. Badan langsung segar!",
    quoteEn:
      "A 4-hour transit? Hitting the gym here is the solution. Complete equipment, hotel-class showers. Instantly refreshing!",
    name: "Dian Pratiwi",
  ...
  {
    quote:
      "Sangat rekomended untuk event korporat. Panitia dibantu dari awal, parkir luas, dan lapangannya terang benderang. Mantap!",
    quoteEn:
      "Highly recommended for corporate events. Helped from the start, ample parking, brightly lit courts. Excellent!",
    name: "Reza Kurniawan",
    role: "HRD Manager",
    roleEn: "HR Manager",
    rating: 5,
  ...
  {
    quote:
      "Court badminton A maupun B kondisinya sama-sama oke. Sudah 3 bulan rutin main di sini, belum pernah kecewa. Booking online-nya simpel banget.",
    quoteEn:
      "Both badminton courts are in great condition. Playing here for 3 months, never disappointed. The online booking is super simple.",
    name: "Siti Rahayu",
  ...
  {
    quote:
      "Harga member bulanan sangat worth it. Datang setiap hari ke gym, fasilitas loker aman, dan staffnya ramah. Paling recommended!",
    quoteEn:
      "Monthly membership price is very worth it. Daily gym visits, secure lockers, friendly staff. Highly recommended!",
    name: "Ahmad Fauzi",
    role: "Member Gym",
    roleEn: "Gym Member",
    rating: 5,
  ...
  {
    quote:
      "Lapangan Multiguna-nya serbaguna banget — minggu ini main basket, minggu depan futsal. Harga per jam kompetitif. Cocok buat komunitas kami.",
    quoteEn:
      "The multipurpose court is super versatile — basketball this week, futsal next week. Competitive hourly rates. Perfect for our community.",
    name: "Rina Wulandari",
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 mt-auto">
                <div
                  className={`w-12 h-12 rounded-full bg-gradient-to-br ${testimonial.color} flex items-center justify-center font-black text-white text-sm shadow-lg`}
                >
                  {testimonial.avatar}
                </div>
                <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center min-h-[calc(100vh-80px)]">
            ...
            <div className="max-w-xl lg:max-w-none animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary font-bold text-sm mb-8 shadow-sm">
                ...
                {t(
                  "Buka Sekarang · 06:00 – 22:00 WIB",
                  "Open Now · 06:00 – 22:00 WIB",
                )}
              </div>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tight text-slate-900 dark:text-white leading-[1.05] mb-6">
                {t("Sport Center Bandara", "Sport Center Bandara")}
                <br />
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {[
                  ...
                  <span
                    key={i}
                    className="px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-semibold border border-slate-200 dark:border-slate-700"
                  >
                    {chip}
                  </span>
                ))}
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-10">
                <Button
                  size="lg"
                  className="h-13 px-8 text-base font-black shadow-xl shadow-primary/30 hover:shadow-primary/45 hover:-translate-y-0.5 transition-all rounded-full group"
                  asChild
                >
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className="h-13 px-8 text-base font-bold rounded-full border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                  asChild
                >
                  <Link href="/facilities">
              ...
              <div className="flex items-center gap-5 pt-2 border-t border-slate-100 dark:border-slate-800">
                <div className="flex -space-x-3">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className="w-9 h-9 rounded-full border-2 border-white dark:border-slate-950 bg-slate-100 overflow-hidden shadow-sm"
                    >
                      <img
                ...
                <div>
                  ...
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">
                    {t(
                      "dari 1.200+ member aktif",
                      "from 1,200+ active members",
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="relative animate-in fade-in slide-in-from-right-8 duration-900 delay-150 lg:py-8">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-black text-sm text-slate-900 dark:text-white">
                        {t("Buka Hari Ini", "Open Today")}
                      </div>
                      <div className="text-xs font-semibold text-slate-500">
                        {settings?.openHour || "06:00"} –{" "}
                        {settings?.closeHour || "22:00"} WIB
                      </div>
                    </div>
                  </div>
              ...
              <div className="absolute -top-4 -right-2 md:right-4 flex items-center gap-2.5 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 px-4 py-3">
                ...
                <div>
                  <div className="font-black text-slate-900 dark:text-white text-sm leading-tight">
                    1,200+
                  </div>
                  <div className="text-xs text-slate-500 font-medium">
                    {t("Member Aktif", "Active Members")}
                  </div>
                </div>
              </div>
              ...
              <div className="absolute -bottom-4 -left-2 md:left-4 flex items-center gap-2.5 bg-primary rounded-2xl shadow-xl shadow-primary/25 px-4 py-3">
                ...
                <div className="text-white">
                  <div className="font-black text-sm leading-tight">
                    {t("Fasilitas #1", "#1 Facility")}
                  </div>
                  <div className="text-xs opacity-80 font-medium">
                    {t("Kawasan Bandara", "Airport Area")}
                  </div>
                </div>
              </div>
            </div>
            ...
            {[
              {
                ...
                label: t("Total Booking", "Total Bookings"),
                suffix: "+",
              },
              {
                end: 7,
                label: t("Lapangan Aktif", "Active Courts"),
                suffix: "+",
              },
              {
                end: 1200,
                label: t("Member Aktif", "Active Members"),
                suffix: "+",
              },
              {
                end: 49,
                label: t("Rating Pelanggan", "Customer Rating"),
                suffix: "/5",
              },
            ].map((stat, i) => (
              <div
                key={i}
                className={`text-center ${i === 3 ? "" : "border-r border-slate-200 dark:border-slate-700 last:border-0"}`}
              >
                <AnimatedCounter
                  end={stat.end}
                  label={stat.label}
        ...
        <div className="container px-4 md:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-5xl font-black tracking-tight text-secondary dark:text-white mb-4">
                {t("Fasilitas", "The Best")}{" "}
                <span className="text-primary">
                  {t("Terbaik", "Facilities")}
                </span>{" "}
                {t("Untuk Anda", "For You")}
              </h2>
            ...
            <Button
              ...
              variant="outline"
              asChild
              className="rounded-full hidden md:flex items-center gap-2 border-border font-bold"
            >
              <Link href="/facilities">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {highlightFacilities.map((facility) => (
              <Card
                key={facility.id}
                className="group border-none shadow-lg hover:shadow-xl transition-all duration-500 rounded-3xl overflow-hidden bg-white dark:bg-slate-900 h-full flex flex-col translate-y-0 hover:-translate-y-2"
              >
                <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-4 left-4">
                    <Badge className="bg-primary text-white font-bold uppercase tracking-wider shadow-md border-0">
                      {t("Populer", "Popular")}
                    </Badge>
                  </div>
                ...
                <CardContent className="p-6 md:p-8 flex-1 flex flex-col">
                  <h3 className="text-2xl font-black text-secondary dark:text-white mb-2 line-clamp-1 group-hover:text-primary transition-colors">
                    {facility.name}
                  </h3>
                  <div className="flex items-center gap-2 text-sm font-semibold text-muted-foreground mb-4">
                    <Users className="w-4 h-4" />
                    <span>
                      {t("Kapasitas", "Capacity")} {facility.capacity || 10}{" "}
                      {t("Orang", "People")}
                    </span>
                  </div>
                  <div className="mt-auto pt-6 border-t border-border flex items-center justify-between">
                    ...
                    <Button
                      className="rounded-full shadow-md shadow-primary/20 font-bold"
                      asChild
                    >
          ...
          <Button
            size="lg"
            asChild
            className="w-full mt-8 rounded-xl h-14 font-bold md:hidden text-base"
          >
            <Link href="/facilities">
              {t("Jelajahi Semua Fasilitas", "Explore All Facilities")}
            </Link>
          </Button>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-[#F8FAFC] dark:bg-slate-900/50 border-y border-slate-100 dark:border-slate-800">
        <div className="container px-4 md:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <Button
              size="lg"
              className="rounded-full font-bold shadow-md shadow-primary/20 shrink-0"
              asChild
            >
              <Link href="/facilities">
                {t("Booking Sekarang", "Book Now")}{" "}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="relative animate-in fade-in slide-in-from-left-8 duration-1000">
              ...
              <div className="absolute -bottom-6 -right-2 md:right-6 p-4 rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-border/50 flex items-center gap-3 max-w-[260px]">
                ...
                <div>
                  <div className="font-black text-secondary dark:text-white leading-tight">
                    {t("Dekat Bandara", "Near The Airport")}
                  </div>
                  <div className="text-xs font-semibold text-muted-foreground">
                    {t(
                      "Menit dari Soekarno-Hatta",
                      "Minutes from Soekarno-Hatta",
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="max-w-xl animate-in fade-in slide-in-from-right-8 duration-1000 delay-150">
              ...
              <h2 className="text-3xl md:text-5xl font-black tracking-tight text-secondary dark:text-white leading-[1.1] mb-6">
                {t(
                  "Gedung Olahraga Modern di Jantung",
                  "A Modern Sports Venue in the Heart of",
                )}{" "}
                <span className="text-primary">
                  {t("Kawasan Bandara", "the Airport Area")}
                </span>
              </h2>
              ...
              <div className="space-y-4 mb-10">
                {[
                  t(
                    "Bangunan luas dengan area parkir yang memadai",
                    "Spacious building with ample parking area",
                  ),
                  t(
                    "Akses mudah dari Terminal Bandara Soekarno-Hatta",
                    "Easy access from Soekarno-Hatta Airport Terminals",
                  ),
                  t(
                    "Fasilitas lengkap berstandar profesional",
                    "Complete facilities with professional standards",
                  ),
                ].map((item, i) => (
              ...
              <Button
                size="lg"
                className="h-14 px-8 text-base font-bold shadow-xl shadow-primary/25 rounded-full group"
                asChild
              >
                <Link href="/contact">
                  {t("Lihat Lokasi", "View Location")}{" "}
        ...
        <div className="container px-4 md:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-lg text-muted-foreground font-medium">
              {t(
                ...
                "We are committed to providing the best sports experience from start to finish.",
              )}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {[
              {
                Icon: Trophy,
                title: t("Fasilitas Premium", "Premium Facilities"),
                desc: t(
                  "Standar lapangan dan peralatan profesional yang dirawat ketat setiap hari.",
                  "Professional-standard courts maintained strictly every day.",
                ),
                href: "/facilities",
              },
              {
                Icon: Zap,
                title: t("Booking Online Mudah", "Easy Online Booking"),
                desc: t(
                  "Cek jadwal real-time dan pesan lapangan langsung dari smartphone Anda.",
                  "Check real-time schedules and book courts from your smartphone.",
                ),
                href: "/facilities",
              },
              {
                ...
                desc: t(
                  "Tidak ada biaya tersembunyi. Bayar sesuai harga yang tertera di sistem.",
                  "No hidden fees. Pay exactly the price shown in the system.",
                ),
                href: "/facilities",
              },
              {
                Icon: MapPin,
                title: t("Lokasi Strategis", "Strategic Location"),
                desc: t(
                  "Sangat mudah diakses, berdekatan dengan area Bandara Soekarno-Hatta.",
                  "Very accessible, adjacent to the Soekarno-Hatta Airport area.",
                ),
                href: "/contact",
              },
              {
                Icon: Phone,
                title: t("Customer Support 24/7", "24/7 Customer Support"),
                desc: t(
                  "Tim kami siap membantu Anda kapan saja melalui WhatsApp.",
                  "Our team is ready to help you anytime via WhatsApp.",
                ),
                href: "/contact",
              },
              {
                Icon: ShieldCheck,
                title: t("Pembayaran Aman", "Secure Payment"),
                desc: t(
                  "Transaksi terjamin dengan berbagai metode pembayaran digital yang aman.",
                  "Secure transactions with various digital payment methods.",
                ),
                href: "/terms",
              },
            ].map((feature, i) => (
              <Link
                key={i}
                href={feature.href}
                className="block group bg-white dark:bg-slate-900 p-8 rounded-3xl border border-border/50 shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-primary/30 transition-all duration-300 cursor-pointer"
              >
                ...
                <h3 className="text-xl font-bold text-secondary dark:text-white mb-3 group-hover:text-primary transition-colors duration-300">
                  {feature.title}
                </h3>
          ...
          <div className="container px-4 md:px-8">
            <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
              <div className="max-w-2xl">
                <h2 className="text-3xl md:text-5xl font-black tracking-tight text-secondary dark:text-white mb-4">
                  {t("Promo &", "Promos &")}{" "}
                  <span className="text-primary">{t("Event", "Events")}</span>
                </h2>
                <p className="text-lg text-muted-foreground font-medium">
                  {t(
                    "Jangan lewatkan kesempatan hemat dan acara seru dari kami bulan ini.",
                    "Don't miss savings and exciting events from us this month.",
                  )}
                </p>
              </div>
              <Button
                ...
                variant="ghost"
                asChild
                className="font-bold text-primary hover:text-primary hover:bg-primary/10 rounded-full hidden md:flex items-center gap-2"
              >
                <Link href="/promos">
                  {t("Lihat Semua Promo", "View All Promos")}{" "}
                  <ArrowRight className="w-4 h-4" />
                </Link>
              </Button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {promos.slice(0, 2).map((promo) => (
                <div
                  key={promo.id}
                  className="group relative bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-border/50 flex flex-col sm:flex-row shadow-sm hover:shadow-xl transition-all duration-500"
                >
                  ...
                  <div className="w-full sm:w-3/5 p-6 md:p-8 flex flex-col justify-center">
                    <div className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-4 w-fit bg-primary/10 text-primary">
                      {promo.type === "promo"
                        ...
                        : t("Acara Spesial", "Special Event")}
                    </div>
                    <h3 className="font-black text-2xl text-secondary dark:text-white mb-3 line-clamp-2">
                      {promo.title}
                    </h3>
                    ...
                    <div className="mt-auto">
                      <Button
                        asChild
                        className="rounded-full font-bold shadow-md shadow-primary/20 w-full sm:w-auto"
                      >
                        <Link href="/promos">
                          {t("Ambil Promo", "Get Promo")}
                        </Link>
                      </Button>
                    </div>
                  </div>
      ...
      <section className="py-20 md:py-28 bg-[#F8FAFC] dark:bg-slate-950 relative overflow-hidden">
        ...
        <div className="container relative z-10 px-4 md:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-black tracking-tight text-secondary dark:text-white mb-4">
              {t("Akses Tak Terbatas.", "Unlimited Access.")}{" "}
              <span className="text-primary">
                {t("Harga Terjangkau.", "Affordable Price.")}
              </span>
            </h2>
            <p className="text-lg text-muted-foreground font-medium">
              {t(
            ...
            {[
              {
                ...
                desc: t(
                  "Akses gym harian tanpa batas reservasi",
                  "Daily gym access without reservation",
                ),
                price: "Rp 300.000",
                ...
                perks: [
                  t(
                    "Akses Gym / Fitness setiap hari",
                    "Gym / Fitness access every day",
                  ),
              ...
              },
              {
                ...
                desc: t(
                  "Akses penuh semua fasilitas + diskon booking",
                  "Full access to all facilities + booking discount",
                ),
                price: "Rp 500.000",
                period: t("/bulan", "/month"),
                ...
                perks: [
                  ...
                  t(
                    "Prioritas booking jam peak",
                    "Priority booking at peak hours",
                  ),
                  t(
                    "1x booking lapangan gratis/bulan",
                    "1x free court booking/month",
                  ),
                  t(
                    "Akses kelas fitness eksklusif",
                    "Exclusive fitness class access",
                  ),
                ],
              ...
              },
              {
                ...
                desc: t(
                  "Paket korporat untuk tim Anda (min. 5 orang)",
                  "Corporate package for your team (min. 5 people)",
                ),
                price: "Rp 350.000",
              ...
              <div
                ...
                className={`relative rounded-3xl bg-white dark:bg-slate-900 border-2 ${plan.color} p-8 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                  plan.featured
                    ? "shadow-2xl shadow-primary/20 scale-[1.02] md:scale-105"
                    : "shadow-sm"
                }`}
              >
                {plan.featured && (
                ...
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${plan.featured ? "bg-primary text-white shadow-lg shadow-primary/30" : "bg-primary/10 text-primary"}`}
                >
                  {plan.icon}
                </div>

                <div className="mb-1 text-xs font-black uppercase tracking-widest text-muted-foreground">
                  {plan.tier}
                </div>
                <div className="text-2xl font-black text-secondary dark:text-white mb-1">
                  {plan.price}
                  <span className="text-sm font-semibold text-muted-foreground">
                    {plan.period}
                  </span>
                </div>
                <p className="text-sm text-muted-foreground font-medium mb-6 pb-6 border-b border-border">
                  {plan.desc}
                </p>
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {plan.perks.map((perk, j) => (
                    <li
                      key={j}
                      className="flex items-start gap-2.5 text-sm font-medium text-secondary dark:text-white"
                    >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 md:gap-12 text-center">
            {[
              {
                Icon: CalendarCheck,
                text: t("Batalkan kapan saja", "Cancel anytime"),
              },
              {
                Icon: ShieldCheck,
                text: t(
              ...
              },
              {
                Icon: TrendingUp,
                text: t(
                  "Sudah 1.200+ member aktif",
                  "Already 1,200+ active members",
                ),
              },
            ].map(({ Icon, text }, i) => (
              <div
                key={i}
                className="flex items-center gap-2.5 text-muted-foreground font-semibold text-sm"
              >
