...
**How to apply:** Simpan `entry_id` pada row payment mirror, gunakan correlation ID per payment, dan retry hanya untuk status `unposted` atau `failed`; status `posted` harus mengembalikan entry yang sama.