...
- [API bundle schema rebuild](api-bundle-schema-rebuild.md) — shared database schema changes require rebuilding the API workflow before runtime responses reflect new fields.
...
- [Gym membership usage bookings](gym-membership-usage-bookings.md) — member check-ins mirror into zero-value confirmed bookings; exclude mirrors from WhatsApp rekap to avoid double counting.
...
- [Development migration ordering](startup-migration-order.md) — wait for an app route, not the exempt health probe, before runtime feature smoke tests.
...
- [WA name-before-date invariant](wa-name-before-date.md) — collect booking name before date; reject retried menu commands when the session is already at ask_name.
- [Fonnte Markdown footer](fonnte-markdown-footer.md) — webhook echoes may wrap “Sent via fonnte.com” in Markdown underscores after a quoted message.
